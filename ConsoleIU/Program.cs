...
namespace ConsoleUI
{
    class Program
    {
        ...
        private static void UserAddition(UsersManager userManager)
        {
        ...
        private static void GetAllUserList(UsersManager usersManager)
        {
        ...
        private static void CarListByColor(CarManager carManager)
        {
            ...
            foreach (var car in carManager.GetCarDetail(I => I.ColorId == colorIdForCarList).Data)
            {
                Console.WriteLine($"{car.CarId}\t{car.ColorName}\t\t{car.BrandName}\t\t{car.ModelYear}\t\t{car.DailyPrice}\t\t{car.Description}");
            }
        }
